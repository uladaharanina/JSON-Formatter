...
export const InputSeparator: React.FC<Props>  = ({insertNewData, removeData}) => {
    ...
    return(
        <div>
            ...
            <div className=" my-5 grid-cols-2">
                {
                    currentInputList && (
                        currentInputList.map((item, index) => (
                            <div key={index} 
                            className="py-2 px-5 my-2 bg-blue-300  text-black mx-1 rounded-md shadow-lg hover:bg-blue-500  cursor-pointer
                            transition-all duration-300 flex items-center gap-3 justify-between">
                                {item}
